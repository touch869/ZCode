 ...
 *
 * 抽取原因：最初是 desktopContextPromptRollout 与 rendererActionTraceRollout 共享同一套
 * /api/v1/client/configs 旁路请求机制，只有 `data.configs.<key>` 的解析不同；复制两份
 * 170 行机制代码会让超时/TTL 语义悄悄分叉。
 * 遥测移除（P1）后 rendererActionTraceRollout 已删除，本机制层现在只服务
 * desktopContextPromptRollout；抽取本身仍成立，不做无谓回并。
 *
